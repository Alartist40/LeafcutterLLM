...
use leafcutter::kernels::q6_k::Matrix as Q6Matrix;
use leafcutter::kernels::q6_k_gemm::q6_k_matmul_transposed_b;

fn main() {
    let path = std::env::args().nth(1).unwrap();
    let file = GGUFile::open(&path).unwrap();
    
    let info = file.get_tensor_info("output.weight").unwrap();
    ...
    let raw = file.get_tensor_raw("output.weight").unwrap();
    ...
    let deq_nk = q6.dequantize(); // [n, k] row-major
    ...
    q6_k_matmul_transposed_b(&a, &q6, &mut c, m, k, n);
    ...
    println!("output.weight: shape={:?} max_diff={:.6} cos_sim={:.6}", shape_gguf, max_diff, dot / (norm_c * norm_e));
}
