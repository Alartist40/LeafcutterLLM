...
fn main() {
    ...
    let qtensor = weights.get("self_attn.o_proj.weight").expect("Missing o_proj");
    ...
    let raw = model.file.get_tensor_raw("blk.0.attn_output.weight").expect("Missing tensor");
    let info = model.file.get_tensor_info("blk.0.attn_output.weight").expect("Missing info");
    let count: usize = info.dimensions.iter().map(|&d| d as usize).product();
    let mut deq = vec![0.0f32; count];
    leafcutter::kernels::dequantize_q4_k(raw, &mut deq);
