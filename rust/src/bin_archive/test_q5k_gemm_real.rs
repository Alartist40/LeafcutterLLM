...
use leafcutter::model::loader::GGUFModel as Loader;
...
fn main() {
    ...
    // Load the quantized tensor through the loader (uses from_q5_k_only)
    let weights = model.load_layer(0).expect("Failed to load layer");
    let qtensor = weights.get("mlp.gate_proj.weight").expect("Missing gate");
    ...
    // Method 1: Native GEMM
    let c1 = atensor.matmul(qtensor);
    
    // Method 2: Dequantize then matmul
    let raw = model.file.get_tensor_raw("blk.0.ffn_gate.weight").expect("Missing tensor");
    let info = model.file.get_tensor_info("blk.0.ffn_gate.weight").expect("Missing info");
    let count: usize = info.dimensions.iter().map(|&d| d as usize).product();
    let mut deq = vec![0.0f32; count];
    leafcutter::kernels::dequantize_q5_k(raw, &mut deq);
